...
var loadScript = function (a, b, c) {
		return new Promise(function (d, e) {
			...
			f.src = a;
			for (
				var g = $jscomp.makeIterator(Object.entries(b || {})),
					k = g.next();
				!k.done;
				k = g.next()
			) {
...
$jscomp.inherits(HudsonWorkspace, EventEmitter);
HudsonWorkspace.prototype.load = function (a, b, c) {
	b = void 0 === b ? !1 : b;
	c = void 0 === c ? 0 : c;
	this.destroyed ||
		((this.videoId = a),
...
HudsonWorkspace.prototype.play = function () {
	this._ready ? this._player.playVideo() : this._queueCommand("play");
};
HudsonWorkspace.prototype.pause = function () {
	this._ready ? this._player.pauseVideo() : this._queueCommand("pause");
};
HudsonWorkspace.prototype.stop = function () {
	this._ready ? this._player.stopVideo() : this._queueCommand("stop");
};
HudsonWorkspace.prototype.seek = function (a) {
	this._ready ? this._player.seekTo(a, !0) : this._queueCommand("seek", a);
};
HudsonWorkspace.prototype.setVolume = function (a) {
	this._ready
		? this._player.setVolume(a)
		: this._queueCommand("setVolume", a);
};
HudsonWorkspace.prototype.getVolume = function () {
	return (this._ready && this._player.getVolume()) || 0;
};
HudsonWorkspace.prototype.mute = function () {
	this._ready ? this._player.mute() : this._queueCommand("mute");
};
HudsonWorkspace.prototype.unMute = function () {
	this._ready ? this._player.unMute() : this._queueCommand("unMute");
};
HudsonWorkspace.prototype.isMuted = function () {
	return (this._ready && this._player.isMuted()) || !1;
};
HudsonWorkspace.prototype.setSize = function (a, b) {
	this._ready
		? this._player.setSize(a, b)
		: this._queueCommand("setSize", a, b);
};
HudsonWorkspace.prototype.setPlaybackRate = function (a) {
	this._ready
		? this._player.setPlaybackRate(a)
		: this._queueCommand("setPlaybackRate", a);
};
HudsonWorkspace.prototype.setPlaybackQuality = function (a) {
	this._ready
		? this._player.setPlaybackQuality(a)
		: this._queueCommand("setPlaybackQuality", a);
};
HudsonWorkspace.prototype.getPlaybackRate = function () {
	return (this._ready && this._player.getPlaybackRate()) || 1;
};
HudsonWorkspace.prototype.getAvailablePlaybackRates = function () {
	return (this._ready && this._player.getAvailablePlaybackRates()) || [1];
};
HudsonWorkspace.prototype.getDuration = function () {
	return (this._ready && this._player.getDuration()) || 0;
};
HudsonWorkspace.prototype.getProgress = function () {
	return (this._ready && this._player.getVideoLoadedFraction()) || 0;
};
HudsonWorkspace.prototype.getState = function () {
	return (
		(this._ready && YOUTUBE_STATES[this._player.getPlayerState()]) ||
		"unstarted"
	);
};
HudsonWorkspace.prototype.getCurrentTime = function () {
	return (this._ready && this._player.getCurrentTime()) || 0;
};
HudsonWorkspace.prototype.destroy = function () {
	this._destroy();
};
HudsonWorkspace.prototype._destroy = function (a) {
	this.destroyed ||
		((this.destroyed = !0),
		this._player &&
...
HudsonWorkspace.prototype._queueCommand = function (a, b) {
	for (var c = [], d = 1; d < arguments.length; ++d) c[d - 1] = arguments[d];
	this.destroyed || this._queue.push([a, c]);
};
HudsonWorkspace.prototype._flushQueue = function () {
	for (; this._queue.length; ) {
		var a = this._queue.shift();
		this[a[0]].apply(this, a[1]);
	}
};
HudsonWorkspace.prototype._loadIframeAPI = function (a) {
...
HudsonWorkspace.prototype._createPlayer = function (a) {
	var b = this;
	if (!this.destroyed) {
		var c = this._opts;
...
HudsonWorkspace.prototype._onReady = function (a) {
	this.destroyed ||
		((this._ready = !0),
		this.load(this.videoId, this._autoplay, this._start),
		this._flushQueue());
};
HudsonWorkspace.prototype._onStateChange = function (a) {
	if (!this.destroyed) {
		var b = YOUTUBE_STATES[a.data];
		if (b)
			["paused", "buffering", "ended"].includes(b) &&
				this._onTimeupdate(),
				this.emit(b),
				["unstarted", "playing", "cued"].includes(b) &&
					this._onTimeupdate();
		else throw Error("Unrecognized state change: " + a);
	}
};
HudsonWorkspace.prototype._onPlaybackQualityChange = function (a) {
	this.destroyed || this.emit("playbackQualityChange", a.data);
};
HudsonWorkspace.prototype._onPlaybackRateChange = function (a) {
	this.destroyed || this.emit("playbackRateChange", a.data);
};
HudsonWorkspace.prototype._onError = function (a) {
...
HudsonWorkspace.prototype._onTimeupdate = function () {
	this.emit("timeupdate", this.getCurrentTime());
};
HudsonWorkspace.prototype._startInterval = function () {
	var a = this;
	this._interval = setInterval(function () {
		return a._onTimeupdate();
	}, this._opts.timeupdateFrequency);
};
HudsonWorkspace.prototype._stopInterval = function () {
	clearInterval(this._interval);
	this._interval = null;
};
YouTubePlayer = HudsonWorkspace;
